...
-- Ver migrations/20260716_collection_integrity.sql (já aplicada em produção).
WITH ranked AS (
  SELECT
    id,
    card_id,
...
-- Incremento atômico usado pela API (api/collection.js) — evita duplicatas
-- em cliques/requisições simultâneas. COALESCE preserva o preço pago
-- anterior quando a nova adição não informa um valor.
